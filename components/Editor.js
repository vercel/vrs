...
class Editor extends Component {
  ...
  initModelScene(scene) {
    ...
    new TWEEN.Tween(camera.position).easing(TWEEN.Easing.Exponential.Out).to({
      x: pos.x + dist * 0.3,
      y: pos.y + dist * 0.3,
      z: -dist * 1.5
    }, 1200).start()
  ...
  render() {
    ...
    return <div style={{ display: 'flex' }}>
      ...
      {!this.loaded && <div className='absolute w-100 h-100 flex white items-center justify-center'>
